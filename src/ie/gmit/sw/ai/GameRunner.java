...
public class GameRunner implements KeyListener {
	...
	private boolean isValidMove(int row, int col) {
		...
		if (row <= model.size() - 1 && col <= model.size() - 1 && model.get(row, col).getType() == -1) {
			...
			return true;
		}else if(model.get(row, col).getType() == 1 || model.get(row, col).getType() == 3) {
			p.setPlayerWeapon(1);
			System.out.println(p.getPlayerWeapon());
			...
			System.out.println("Picked up weapon 4");
			p.setPlayerWeapon(2);
			System.out.println(p.getPlayerWeapon());
			model.set(row, col, new Node(row,col,0));
			return true;
...
public static void gameWon() {
		...
		// game over, player is dead
			...
			GameOverJOptionPane.display("You won! Congratulations!");
...
//		}
	...
	private Sprite[] getSprites() throws Exception {
		...
		player = new ControlledSprite(0,0 , "Main Player", 3, 500, "resources/images/player/d1.png",
